...
library(ggplot2)
library(tidyr)
library(tibble)  # <-- AÑADE ESTA LÍNEA


# Matriz de correlaciones solo con INDFMPIR
cor_matrix <- dd %>%
	select(where(~is.numeric(.x) && !inherits(.x, c("Period", "hms", "difftime")))) %>%
	cor(use = "pairwise.complete.obs")

# Extraer solo la fila/columna de INDFMPIR
cor_indfmpir <- cor_matrix["INDFMPIR", ] %>%
	as.data.frame() %>%
	rownames_to_column("variable") %>%
	rename(correlation = 2) %>%
	filter(variable != "INDFMPIR") %>%  # Quitar la correlación consigo misma
	arrange(desc(abs(correlation)))  # Ordenar por correlación absoluta

# Ver tabla
print(cor_indfmpir)

# Gráfico de barras
ggplot(cor_indfmpir, aes(x = reorder(variable, correlation), y = correlation, fill = correlation)) +
	geom_col() +
	geom_text(aes(label = sprintf("%.2f", correlation)), hjust = 1.2, size = 3) +
	coord_flip() +
	scale_fill_gradient2(low = "#d7191c", mid = "#ffffbf", high = "#2c7bb6", midpoint = 0) +
	theme_minimal() +
	labs(title = "Correlaciones con INDFMPIR",
	     x = "Variable",
	     y = "Correlación")
